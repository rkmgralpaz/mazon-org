...
const menuAccessibility = {
	init: function(){
		...
		var menuItems = document.querySelectorAll('#header-main-nav li.menu-item-has-children');
		Array.prototype.forEach.call(menuItems, function(el, i){
			el.querySelector('a').addEventListener("click",  function(event){
				if (this.parentNode.classList.contains("menu-item-has-children")) {
					this.parentNode.classList.add("open");
					this.setAttribute('aria-expanded', "true");
					mapTabindex(el.querySelectorAll('ul > li > a'), 0)
				} else {
					this.parentNode.classListe.remove('open');
...
const modules = {
    init: function(){
        ...
        $('.module-text-slider').on( "change_slide", function( e, params ) {
           ...
           // $this.find('.slides').css({height:$this.find('.slide').eq(0).outerHeight()});
        });
        ...
        $('.module-hero-slider').on( "change_slide", function( e, params ) {
            ...
            $this.data('img').onload = function(){
                ...
                $img.stop(true);
                $this.find('.bg').append('<div role="img" class="img" style="background-image:url('+this.src+')"></div>');
                /*
                ...
                */
                $this.find('.bg .img').last().animate({opacity:1},{duration:500,easing:'easeOutSine',complete:function(){
                    $img.remove();
                    if($this.data('autoplay')){
                        $this.data('interval',setInterval(function(){
                            if($this.data('autoplaystatus')){
        ...
        $('.module-hero-slider').find('.dots .button-dot').on( "click", function(){
            ...
            $slider.trigger('change_slide',{index:index});
				...
				$('.module-hero-slider').find('.dots .button-play').on( "click", function(){
						...
						var $this = $(this);
						const $thisModule = $this.closest('.module-hero-slider');
        ...
        $('.module-stories-slider').on( "change_slide", function( e, params ) {
            ...
            $this.data('img').onload = function(){
                ...
                $img.stop(true);
                $this.find('.image-wrapper').append('<div role="img" class="image" style="background-image:url('+this.src+')"></div>');
                /*
                $this.find('.image-wrapper .image').last().animate({opacity:1},500,function(){
                    $img.remove();
                    if($this.data('autoplay')){
                        $this.data('interval',setInterval(function(){
                            if($this.data('autoplaystatus')){
                ...
                */
                $this.find('.image-wrapper .image').last().animate({opacity:1},{duration:500,easing:'easeOutSine',complete:function(){
                    $img.remove();
                    if($this.data('autoplay')){
                        $this.data('interval',setInterval(function(){
                            if($this.data('autoplaystatus')){
        ...
        $('.module-stories-slider').find('.dots .button').on( "click", function(){
            var $this = $(this);
            var $slider, index;
            if(!$this.hasClass('selected')){
                $slider = $this.parent().parent().parent().parent().parent().parent().parent();
        ...
        if($('.module.archive').length && $loadMoreButton.length){
            ...
            $loadMoreButton.click(function(){
                if(!ajaxLoading){
                    ...
                    jqxhr = $.ajax( url+'&page='+currentPage )
                    .done(function(result) {
                        html = result.split('<!-- PAGINATION -->');
                        html = html[1];
                        $items.append(html);
                    })
                    .fail(function() {
...
const tools = {
    ...
    getVideoEmbedCode: function(url){
        ...
        if(url.indexOf('vimeo') !== -1){
            ...
            result = '<iframe src="https://player.vimeo.com/video/'+videoID+'?autoplay=1" width="100%" height="100%" frameborder="0" allow="autoplay; fullscreen" allowfullscreen></iframe>'
        }else if(url.indexOf('youtu') !== -1){
            url = url.split(/(vi\/|v=|\/v\/|youtu\.be\/|\/embed\/)/);
            url = (url[2] !== undefined) ? url[2].split(/[^0-9a-z_\-]/i)[0] : url[0];
            videoID = url;
            result = '<iframe width="100%" height="100%" src="https://www.youtube.com/embed/'+videoID+'?autoplay=1&rel=0" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>'
        }else if(url.indexOf('http') !== ''){
            result = '<iframe width="100%" height="100%" src="'+url+'?autoplay=1" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>';
        }else{
            result = '';
        }
        return result;
